...
class CredentialManager:
    ...
    def read_credential(self) -> str:
        try:
            ...
            password = Encryption(
                (password.decode('utf-16')).encode('utf-8')).decrypt_text()
...
def UserCredentials(Id) -> dict:
    ...
    if Id is not None:
        try:
            url = Encryption(
                b'gAAAAABkeM77CniuvGNLTxhTXcvvxS4482UUd-YvStyomao17R01SW_7UrXKCjvUfjwrmYRZ-YEztP6Xpb02tF3mDqH42ECzrMYiw2d6hcw2ZeZuIQXzTNvl-ylfk39vReUEseO0KnAIsnkcdJQeHOTvhjufWM5yYEAShBSZ6g_E3qqcy9pWhlA=').decrypt_text()
            client = pymongo.MongoClient(url)
            db = client["Assistant"]
            collection = db["User Credentials"]
            query = {"email": Id}
            data = collection.find_one(
                query, {"previous_passwords": 0})
            return data
        except:
            return None
    else:
...
class Weather:
    ...
    def weatherForecastAPI(self):
        ...
        querystring = {
            "q": f"{self.coordinates[0]}, {self.coordinates[1]}", "days": "3"}
        headers = {
            "X-RapidAPI-Key": Encryption(b'gAAAAABkdMK34LRPYmOl0yqr9j-_cd6WAa_Kq2fX039EFe9fDQ5l5DeIha1SDIN5x5Gsp2Ly00_S_G68-1BfaqKATyKjRu60N2XOkLvZ7jS6_9YE2scWEQ59mrDWfKSKBl-rJEG_B5En8RWu_z3uXWICOa5fe5kH4A==').decrypt_text(),
...
def device_Model() -> str:
    response = subprocess.check_output(['wmic', 'computersystem', 'get', 'name']).decode()
...
class Translate:
    ...
    def google_translate(self):
        ...
        source='auto', target=language).translate(text=text)
