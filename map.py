...
from location import reverseGeocoding
...
class APP(customtkinter.CTk):
    def __init__(self):
        ...
        self.title("Mili")
        self.GUI_geometry()
        self.iconbitmap(application_directory +
                        "\\Data\\Images\\GUI\\logo.ico")
    ...
    def currentLocation(self, latitude, longitude):
        ...
        def changeTileServer(mode):
            if mode == "Default":
                map_widget.set_tile_server(
                    "https://mt0.google.com/vt/lyrs=m&hl=en&x={x}&y={y}&z={z}&s=Ga", max_zoom=22)  # (default)
        ...
        map_widget.pack(side="top", anchor='center', expand=True,
        ...
        viewOption = customtkinter.CTkOptionMenu(footerFrame, values=["Default", "Satellite", "Open Street"], width=200, height=40, font=("Sitka Small", 14, "normal"),
                                                 fg_color="#171717", button_color="#111", button_hover_color="#202020", dropdown_font=("Sitka Small", 14, "normal"), dropdown_fg_color="#303030", dynamic_resizing=True, command=changeTileServer)
        viewOption.pack(side='right', anchor='ne', pady=(0, 60))
        customtkinter.CTkLabel(footerFrame, text="Current Location", text_color="#7f7f7f", fg_color="#252525", font=(
            "Sitka Small", 16, 'normal'), wraplength=350).pack(side='top', anchor='nw', padx=30, pady=5)
        customtkinter.CTkLabel(footerFrame, text=f"{address}", text_color="#fff", fg_color="#252525", font=(
            "Sitka Small", 16, 'normal'), wraplength=350).pack(side='left', anchor='nw', padx=30)
    ...
    def distanceAndRoute(self, query):
        location, route = Route(query).route()
        if location is not None and route is not None:
            ...
            map_widget.set_marker(deg_x=destination[0], deg_y=destination[1], text="Destination")
            ...
            viewOption = customtkinter.CTkOptionMenu(footerFrame, values=["Default", "Satellite", "Open Street"], width=200, height=40, font=("Sitka Small", 14, "normal"),
                                                    fg_color="#171717", button_color="#111", button_hover_color="#202020", dropdown_font=("Sitka Small", 14, "normal"), dropdown_fg_color="#303030", dynamic_resizing=True, command=changeTileServer)
            viewOption.pack(side='right', anchor='ne', pady=(0, 60))
            customtkinter.CTkLabel(footerFrame, text=f"Distance {distance}", text_color="#7f7f7f", fg_color="#252525", font=(
                "Sitka Small", 16, 'normal'), wraplength=350).pack(side='top', anchor='nw', padx=30, pady=5)
            customtkinter.CTkLabel(footerFrame, text=f"Time {time}", text_color="#fff", fg_color="#252525", font=(
                "Sitka Small", 16, 'normal'), wraplength=350).pack(side='left', anchor='nw', padx=30)
            map_widget.add_right_click_menu_command(
                label="Add Marker", command=add_marker_event, pass_coords=True)
